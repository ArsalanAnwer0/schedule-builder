...
scheduleConfigSchema.pre('save', async function() {
...
scheduleConfigSchema.pre('save', function() {
  const { minShiftLength, maxShiftLength, idealShiftLength } = this.shiftPreferences;

  if (minShiftLength > maxShiftLength) {
    throw new Error('minShiftLength cannot be greater than maxShiftLength');
  }

  if (idealShiftLength < minShiftLength || idealShiftLength > maxShiftLength) {
    throw new Error('idealShiftLength must be between minShiftLength and maxShiftLength');
  }
});
...
scheduleConfigSchema.pre('save', function() {
  ...
  if (minWorkersPerShift > maxWorkersPerShift) {
    throw new Error('minWorkersPerShift cannot be greater than maxWorkersPerShift');
  }
});
